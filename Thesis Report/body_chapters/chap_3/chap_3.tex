...
The deviations $\Delta V_{th}$ and $\Delta L_{eff}$ are composed of two independent components: systematic variation, and random variation \cite{Kuhn2011}.
\begin{equation}
...
Systematic variation represents spatially correlated deviations caused by layout-dependent and process-wide effects \cite{Sarangi2008}. Neighboring regions of the chip tend to exhibit similar variation values. Systematic variation is modeled using a multivariate normal distribution with a spherical spatial correlation structure \cite{Sarangi2008}. The correlation between two regions depends only on their Euclidean distance $r$. The spherical correlation function is defined as \cite{Sarangi2008}:
\begin{equation}
\rho(r) =
\begin{cases}
...
Random variation, in contrast, is uncorrelated and occurs at a much finer granularity, reflecting device-level fluctuations such as dopant variation. Both random and systematic components are assumed to follow normal distributions with zero mean \cite{Kuhn2011}. Since these components are independent, the total variation also follows a normal distribution, defined as \cite{Clotho2015}:
\begin{equation}
\sigma_{total} = \sqrt{\sigma_{rand}^2 + \sigma_{sys}^2}
\end{equation}

The parameter values used in this work are taken directly from \cite{Clotho2015} and are summarized as follows:
\begin{itemize}
...
The resulting per-router values of $V_{th}$ and $L_{eff}$ are subsequently used to compute the substrate current ($I_{sub}$), which serves as an intermediate quantity in the HCI aging model. Since PV is static and all other parameters involved in this computation are technology constants, $I_{sub}$ can be precomputed once at initialization. The substrate current is calculated as follows \cite{WhiteBernstein2008}:

\begin{equation}
I_{sub} = C_2 \cdot I_{ds} \cdot \exp\left(\frac{-\phi_i}{q \lambda_e E_m}\right)
\end{equation}

where $C_2$ is a proportionality constant, $I_{ds}$ is the drain current, $\phi_i$ denotes the minimum energy required for a hot electron to initiate impact ionization, $q$ is the elementary charge, $\lambda_e$ is the mean free path of hot electrons, and $E_m$ is the electric field in the channel. To compute $E_m$, a semi-quntitative analytical model is used, expressed as \cite{WhiteBernstein2008}:

\begin{equation}
E_m = \frac{V_{ds} - V_{dsat}}{\sqrt{3 t_{ox} x_j}}
\end{equation}

where $V_{ds}$ denotes the drain-to-source voltage and $V_{dsat}$ corresponds to the voltage at the channel pinch-off point. The term $\sqrt{3 t_{ox} x_j}$ represents a characteristic length associated with the effective thickness of the pinch-off region in the transistor channel. For typical CMOS technologies, this length falls within the range $\sqrt{100\ \text{nm}}$ to $\sqrt{300\ \text{nm}}$ \cite{Clotho2015}.

For short channel devices, $V_{dsat}$ is dependent on the effective gate length and can be modeled as \cite{WhiteBernstein2008}:
...
Temperature is modeled as a slowly varying parameter rather than as a quantity that changes every cycle. In \cite{Clotho2015}, the absolute values of the temperature map are updated every $10^7$ cycles in order to emulate the gradually changing thermal behavior of a running CMP, consistent with prior observations by Mulas et al. \cite{Mulas2009}, that on-chip temperature sensing and thermal evolution occur over millisecond-scale intervals rather than cycle-by-cycle timescales. The same assumption is adopted here. Accordingly, the temperature map is periodically refreshed every $10^7$ cycles, allowing the routing algorithm to respond to long-term changes in the thermal profile without introducing unnecessary per-cycle thermal recomputation. The temperature assigned to each tile is generated as part of a spatial temperature map spanning the range of 65$^\circ$C to 85$^\circ$C, consistent with the range used in \cite{Clotho2015}. In implementation, these values are generated using a random but spatially correlated distribution so that neighboring tiles tend to exhibit similar thermal behavior. Figure \ref{fig:temp_maps} illustrates an example of such a temperature map. This per-tile temperature information is then used directly by the EM and HCI aging models. Since both failure mechanisms accelerate at higher temperatures, tiles operating at elevated temperature accumulate wear more rapidly than cooler tiles under otherwise similar activity conditions.

\section{Aging Models}

\subsubsection{Electromigration (EM)}

Electromigration (EM) is a wearout mechanism in which momentum transfer from conducting electrons causes gradual displacement of metal ions in interconnect wires \cite{Black1969}. Under sustained high current density, this ion migration leads to the formation of voids and hillocks, eventually resulting in open- or short-circuit failures. As technology scales and interconnect dimensions shrink, current densities increase, making EM a critical reliability concern in modern NoCs \cite{Clotho2015}.

The lifetime of an interconnect under EM stress is commonly modeled using Black's equation \cite{Black1969}:
\begin{equation}
MTTF_{EM} = A_{EM} J^{-n} \exp\left(\frac{E_{a,EM}}{kT}\right)
\end{equation}
where $J$ is the current density, $n$ is an empirical exponent (typically close to 2 \cite{Clotho2015}), $E_{a,EM}$ is the activation energy, $T$ is temperature, and $k$ is Boltzman's constant.

Current density is closely related to switching activity \cite{Black1969}. As traffic increases, links are exercised more frequently, leading to a higher rate of current flow through the interconnect. Long router-to-router links tend to carry a larger share of traffic and exhibit higher effective current density compared to shorter intra-router wires. As a result, these links are more susceptible to electromigration-induced degradation.
...
Substituting this relationship into Black's equation yields a simplified proportional model \cite{Clotho2015}:
\begin{equation}
MTTF_{EM}(\alpha_{link}, T) \propto \alpha_{link}^{-2} \cdot \exp\left(\frac{E_{a,EM}}{kT}\right)
\end{equation}

The switching activity $\alpha_{link}$ is derived from link utilization \cite{Clotho2015}:
\begin{equation}
...
Hot-carrier injection (HCI) is a transistor-level aging mechanism caused by high-energy carriers that become trapped in the gate oxide during switching \cite{JEDEC122G}. Over time, this leads to a gradual increase in threshold voltage ($V_{th}$), which slows transistor switching speed and degrades circuit performance. In NoC routers, this degradation is most critical along timing-critical paths, where increased delay can eventually result in timing violations and system failure \cite{Clotho2015}.

The lifetime of a device under HCI stress can be expressed as \cite{JEDEC122G}:
\begin{equation}
MTTF_{HCI}^{DC} = B_{HCI} (I_{sub})^{-N} \exp\left(\frac{E_{a,HCI}}{kT}\right)
\end{equation}
where $B_{HCI}$ is an arbitrary scale factor, $I_{sub}$ is the substrate current during stress, $N$ is an empirical exponent (approximately 1.5 \cite{Clotho2015}), $T$ is the temperature, and $E_{a,HCI}$ is the activation energy.

Since HCI degradation occurs during switching events, its impact under dynamic operation is proportional to switching activity. Incorporating this effect yields the following model for HCI lifetime under switching activity \cite{Clotho2015}:
\begin{equation}
...
Combining these effects yields the final HCI model used in this work, following the formulation in \cite{Clotho2015}:
\begin{equation}
...
The EM and HCI models described above are used both to guide routing decisions and to evaluate system reliability. During routing, the MTTF of candidate next-hop links and routers is computed using recent utilization statistics and current temperature values. These MTTF estimates are normalized relative to the network state and used to define the reward signal for the reinforcement learning policy. As a result, routing decisions are biased toward paths that traverse components with higher estimated lifetime.

To capture current stress conditions, utilization is computed using a windowed formulation rather than cumulative statistics. Every $10^3$ cycles, as opposed to $10^4$ cycles proposed by \cite{Clotho2015}, link and router activity is sampled to obtain a recent utilization rate, defined as the number of events observed within the window divided by the window duration. This produces a measure of instantaneous switching activity that reflects recent traffic behavior.

Using cumulative averages would dilute recent activity with historical behavior. For example, a component that was heavily utilized earlier in the simulation but is currently idle would still appear moderately stressed when using lifetime-averaged statistics. This leads to inaccurate MTTF estimates, as the model would continue to penalize components that are no longer under significant stress. In contrast, the windowed formulation ensures that utilization reflects the current operating conditions of each component. This is consistent with the underlying physics-of-failure models for EM and HCI, where degradation rates depend on the present level of electrical and thermal stress rather than long-term averages. As a result, the routing policy can more accurately identify and avoid currently stressed components.

The choice of a $10^3$ cycle window provides a balance between responsiveness and stability. Smaller windows can introduce noise due to short-term traffic fluctuations, while larger windows may fail to capture changes in load distribution in a timely manner. This design enables the routing policy to respond effectively to dynamic traffic conditions while maintaining stable utilization estimates.

Throughout simulation, these utilization snapshots are updated periodically and used to recompute EM and HCI MTTF estimates as needed. The MTTF values are not maintained as persistent aging state variables; instead, they are derived on demand from the latest utilization statistics, temperature map, and static process-variation parameters. This enables the routing policy to continuously adapt to dynamic traffic conditions. At the end of simulation, the final utilization statistics are used to compute MTTF values for all routers and links. The system lifetime is then defined by the most degraded component, reflecting the fact that failure of a single critical link or router can render the network unusable.

To compare routing algorithms, the acceleration factor (AF) is used as a relative lifetime metric, as defined by \cite{JEDEC122G}:
\begin{equation}
AF_{EM} = \frac{MTTF_{EM}|_{custom\_algo}}{MTTF_{EM}|_{DOR}}, 
\quad
AF_{HCI} = \frac{MTTF_{HCI,PV}|_{custom\_algo}}{MTTF_{HCI,PV}|_{DOR}}
\end{equation}
...
In this work, we adopt Q-learning \cite{Watkins1989}, a value-based RL method. Q-learning estimates the expected cumulative reward of taking an action $a$ in a state $s$, represented by the Q-value function $Q(s,a)$. The optimal policy is obtained by selecting actions that maximize this value.

The Q-values are updated iteratively using the Bellman equation \cite{BellmanDreyfus1962}:
...
In the context of NoCs, routing decisions can be formulated as a sequential decision-making problem, where each router selects the next-hop direction for a packet based on local network conditions. Each router is modeled as an independent RL agent, and routing corresponds to selecting an action that maximizes long-term reward \cite{LARE2023}. Direct application of tabular Q-learning, however, is impractical in NoCs due to the large state-action space, which grows rapidly with network size. This leads to excessive storage requirements and slow convergence, commonly referred to as the curse of dimensionality \cite{Ebrahimi2012}. To address this limitation, this work adopts a linear function approximation approach inspired by LARE \cite{LARE2023}, where Q-values are represented using a compact feature-based model instead of a lookup table.

\subsubsection{State Representation}

Routing decisions are made at each router when multiple minimal paths are available. Instead of representing the state explicitly, each state-action pair $(s,a)$ is encoded using a feature vector $\phi(s,a)$, adapted from \cite{LARE2023}:
...
To reduce hardware overhead, the Q-value is approximated using a linear function \cite{LARE2023}:
...
The reliability component is defined as a weighted sum of normalized EM and HCI MTTF values, adapted from \cite{Clotho2015}:
\begin{equation}
\text{MTTF}_{score} = C_{EM} \cdot MTTF_{EM_{norm}} + C_{HCI} \cdot MTTF_{HCI_{norm}}
\end{equation}

The EM and HCI values are normalized across candidate actions:
\begin{equation}
MTTF_{EM_{norm}} = \frac{MTTF_{EM}}{\max(MTTF_{EM}^{(h)}, MTTF_{EM}^{(v)})}
\end{equation}

\begin{equation}
MTTF_{HCI_{norm}} = \frac{MTTF_{HCI}}{\max(MTTF_{HCI}^{(h)}, MTTF_{HCI}^{(v)})}
\end{equation}

where the superscripts $(h)$ and $(v)$ denote the horizontal and vertical candidate next hops, respectively.

To dynamically emphasize the more critical aging mechanism, and to encourage the agent to route packets around heavily stressed components, adaptive weights are computed using the model proposed in \cite{Clotho2015}:

\begin{equation}
C(x) =
\frac{\exp\left(8 \cdot \frac{x - x_{max}}{x_{max} - x_{min}}\right) - \exp(-8)}
{1 - \exp(-8)}
\end{equation}
...
The parameter vector $\theta$ is updated using temporal-difference learning. The update rule for linear function approximation is given by \cite{LARE2023}:
